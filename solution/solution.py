...
class LLMJudge:
    ...
    def score_response(
        ...
        """
        prompt = (
            "Score every criterion from 0.0 to 1.0. Return a JSON object "
            "mapping criterion names to scores. Do not reward length alone.\n"
            f"Question: {question}\nAnswer: {answer}\n"
        ...
        candidates = parsed.get("scores", parsed) if isinstance(parsed, dict) else {}
        scores: dict[str, float] = {}
        for criterion in rubric:
            try:
                value = float(candidates.get(criterion, 0.5))
            except (TypeError, ValueError):
                value = 0.5
            scores[criterion] = max(0.0, min(1.0, value))
        return {"scores": scores, "reasoning": str(raw)}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        values: list[float] = []
        positioned: list[tuple[Any, float]] = []
        ...
        first = [
            score for position, score in positioned
            if position in (0, 1, "first", "A")
        ]
        later = [
            score for position, score in positioned
            if position not in (None, 0, 1, "first", "A")
        ]
        positional_bias = bool(
            first and later
            and sum(first) / len(first) - sum(later) / len(later) > 0.05
        )
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            suggestion = (
                ...
                else "Review this case and add it to the regression dataset"
            )
            rows.append(
    ...
    def generate_improvement_suggestions(
        ...
        fixes = {
            "hallucination": "Add a faithfulness guardrail for unsupported claims",
            "irrelevant": "Improve intent detection and add relevant few-shot examples",
            "incomplete": "Add answer checklists and retrieve all required evidence",
            "off_topic": "Clarify scope and route requests before generation",
            "refusal": "Review guardrails and add examples of allowed requests",
            "unknown": "Review and label uncategorized failures",
        }
        ...
        suggestions = [
            fixes.get(failure_type, fixes["unknown"])
            for failure_type in ordered_types
        ]
        general = [
            "Add failed cases to the golden dataset and CI regression tests",
            "Inspect retrieval traces and tune chunking, filtering, and ranking",
            "Calibrate thresholds against independently reviewed human labels",
        ]
